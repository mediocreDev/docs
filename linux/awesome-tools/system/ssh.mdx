...
if ! command -v sshd >/dev/null 2>&1; then
  echo "⚠️  OpenSSH server not found, installing..."
  sudo apt update && sudo apt install -y openssh-server
else
  echo "✅ sshd found: $(which sshd)"
fi

echo "🧱 Checking for socket-activation mode..."
if systemctl is-active --quiet ssh.socket; then
  echo "⚙️  Disabling socket activation (so custom port works)..."
  sudo systemctl disable --now ssh.socket
  sudo systemctl enable --now ssh.service
else
  echo "✅ Socket activation not in use or already disabled."
fi

echo "📝 Backing up sshd_config..."
sudo cp "$SSH_CONFIG" "${SSH_CONFIG}.bak.$(date +%F_%T)"

echo "✏️  Updating SSH port and disabling root login..."
...
sudo sshd -t && echo "✅ sshd configuration OK"
...
echo "🔎 Checking new port..."
if sudo ss -tuln | grep ":$NEW_PORT" >/dev/null; then
  echo "✅ SSH is now listening on port $NEW_PORT"
else
  echo "⚠️  SSH not yet listening on $NEW_PORT — check /etc/ssh/sshd_config"
fi

SERVER_IP=$(hostname -I | awk '{print $1}')
echo ""
echo "🧪 Test new connection in another terminal:"
echo "    ssh -p $NEW_PORT $(whoami)@$SERVER_IP"
echo ""
echo "⚠️  Keep this session open until you confirm new access works!"
echo ""

read -p "Press Enter once you successfully connect via new port..."

echo "🚫 Removing old port 22 from firewall..."
sudo ufw delete allow 22/tcp || true

echo "🛡️ Installing Fail2Ban (optional but recommended)..."
sudo apt install -y fail2ban

echo ""
echo "✅ Done! SSH hardened and running on port $NEW_PORT"
echo "   • Root login disabled"
echo "   • Fail2Ban active"
echo "   • Socket activation off (using ssh.service)"
```
...
echo "📦 Installing semanage, just in case"
...
echo "✏️ Editing SSH config - change port"
sudo sed -i "s/^#Port.*/Port ${NEW_PORT}/" $SSHD_CONFIG
sudo sed -i "s/^Port .*/Port ${NEW_PORT}/" $SSHD_CONFIG

#echo "✏️ Editing SSH config - disable root login"
#if [ "$DISABLE_ROOT_LOGIN" = true ]; then
#    sudo sed -i "s/^#PermitRootLogin.*/PermitRootLogin no/" $SSHD_CONFIG
#    sudo sed -i "s/^PermitRootLogin.*/PermitRootLogin no/" $SSHD_CONFIG
#fi

#echo "✏️ Editing SSH config - disable authentication via password"
#if [ "$DISABLE_PASSWORD_AUTH" = true ]; then
#    sudo sed -i "s/^#PasswordAuthentication.*/PasswordAuthentication no/" $SSHD_CONFIG
#    sudo sed -i "s/^PasswordAuthentication.*/PasswordAuthentication no/" $SSHD_CONFIG
#fi

echo "⚙️ Configuring SELinux SSH port permission"
